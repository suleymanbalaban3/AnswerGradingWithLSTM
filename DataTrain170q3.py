...
data = [ ( row["sentence"] , row["label"]  ) for row in csv.DictReader(open("answer170WithRandomShuffle3.txt"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
#random.shuffle( data )
print ("Reading text data for classification and building representations...")
data1 = [ ( row["sentence"] , row["label"]  ) for row in csv.DictReader(open("dataAllTester3.txt"), delimiter='\t', quoting=csv.QUOTE_NONE) ]
...
print(test_labels)
print(results)
model.save_weights('q170DataBatch16Epoch40Shuffledq3.h5')
K.clear_session()	
